...
use App\Mail\MailFive;
use App\Mail\MailFour;
use App\Mail\MailThree;
use App\Mail\MailTwo;
use App\Mail\MailOne;
...
class BorrowersController extends Controller {
    ...
    public function view($id) {
        ...
        $borrowers = Borrowers::find($id);
		$borrowers_amount = BN::where('borrower_id', $id)->orderBy('id', 'desc')->first();
        if( empty($borrowers) ) {
            return Redirect::to('borrowers')->withErrors("No Lander information found.");
        }
        ...
        return view('borrowers.view',$data, compact('borrowers_amount'));
    }
	...
	public function mail($borrower_id,$id){
		$borrower = Borrowers::find($borrower_id);
		$name = $borrower->first_name.' '.$borrower->sur_name;
		$loan = $borrower->loan_amount-$borrower->repay;
		$account_number = $borrower->account_no;
		$bank_short_code = $borrower->bank_short_code;
		$address = $borrower->address1;
		$email = $borrower->email_address;
		
		
		if($id == 5){
			$data = ['loan' => $loan, 'name' =>$name];
			Mail::to('itsumrat@gmail.com')->send(new MailFive($data));

			return back()->withSuccess("Mail number five has been sent successfully!");
		}
		else if($id == 4){
			$data = ['loan' => $loan, 'name' =>$name,'account_number' => $account_number, 'bank_short_code' => $bank_short_code];
			Mail::to('itsumrat@gmail.com')->send(new MailFour($data));
			return back()->withSuccess("Mail number four has been sent successfully!");
		}
		else if($id == 3){
			$data = ['loan' => $loan, 'name' =>$name,'account_number' => $account_number, 'bank_short_code' => $bank_short_code];
			Mail::to('itsumrat@gmail.com')->send(new MailThree($data));
			return back()->withSuccess("Mail number three has been sent successfully!");
		}
		else if($id == 2){
			$data = ['loan' => $loan, 'name' =>$name];
			Mail::to('itsumrat@gmail.com')->send(new MailTwo($data));
			return back()->withSuccess("Mail number two has been sent successfully!");
		}
		else if($id == 1){
			$data = ['loan' => $loan, 'name' =>$name, 'address' => $address, 'email' => $email ];
			Mail::to('itsumrat@gmail.com')->send(new MailOne($data));
			return back()->withSuccess("Mail number one has been sent successfully!");
		}
	}
	public function maildefault(Request $request){
		$data =['sub' => $request->subject,'body' => $request->body,'attachment' => null];
		if ($request->hasFile('attachment')){
			$data =['sub' => $request->subject,'body' => $request->body, 'attachment' => $request->file('attachment'), 'attachmentName' => $request->file('attachment')->getClientOriginalName()];
		}
		
			Mail::to('mollameehedi@gmail.com')->send(new MailDefault($data));
		return back()->withSuccess("Default Mail has been sent successfully!");
	}
