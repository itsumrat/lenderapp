...
<div class="header" style="padding:0px">
    <div id="ember1003" class="pagetips-title float-right ember-view">
      <a style="font-size: 14px" href="{{Request::root()}}/borrowers/add">
        Add New Lender
      </a>
    ...
    <h3 class="float-left">All Lenders</h3>
...
<div class="table-responsive overflow-initial customviews-table">
    <table id="ember465" class="header-fixed table zi-table table-hover ember-view">
      ...
      <tbody>
        ...
        <tr class="ember-view">
            <td><a href="{{url('borrowers/view/'.$list->id)}}">{{$list->reg_no}}</a></td>
            <td>{{$list->first_name}}({{$list->sur_name}})</td>
            <td>{{$list->email_address}}</td>
            <!--<td>{{$list->mobile}}</td>-->
            <td>{{$list->city}}</td>
            <td class="text-center">{{$loanByID[$list->id]}}</td>
            <td class="text-center">{{$payable[$list->id]}}</td>
        </tr>
        @endforeach
      </tbody>
   </table>
